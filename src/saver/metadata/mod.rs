...
mod test {
    ...
    use super::*;
    use crate::structs::tags::{CompressionMethod, PhotometricInterpretation};
    ...
    fn test_save_header_small_le() {
        ...
        assert_eq!(Tiff::from_header(&buf).unwrap(), tiff);
    }
    ...
    fn test_save_tiff_roundtrip_single_tile_hack() {
        ...
        // assert_eq!(&out, &[]);
        let mut read = Tiff::from_header(&out).unwrap();
        let (mut read_ifd, zero) = read
            .ifd_loader(
                &out[read.next_ifd_offset().unwrap() as usize..],
