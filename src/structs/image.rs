...
impl Image {
    ...
    pub fn chunk_offset(&self, index: usize) -> TiffResult<&u64> {
        self.chunk_offsets.get(index).ok_or(TiffError::LimitsExceeded)
    }
    ...
    pub fn from_ifd(
        ...
        // {
        //     let vec
        //      = ifd.find_tag(Tag::JPEGTables)?.unwrap().into_u8_vec()?;
        //     if vec.len() < 2 {
