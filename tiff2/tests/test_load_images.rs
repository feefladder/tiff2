    ...
    for p in fs::read_dir("./tests/img/gdal/out").unwrap() {
        ...
        for (_offset, img) in d.images {
            println!(
                ...
                img.tile_opts.image_width, img.tile_opts.image_height, img.tile_opts.predictor
            );
            ...
            let img_bytes = (u64::from(img.tile_opts.image_width)
                * u64::from(img.tile_opts.image_height)
                * u64::from(img.tile_opts.bits_per_sample)
                * u64::try_from(img.tile_opts.samples_per_pixel()).unwrap())
            .div_ceil(8);
            ...
            for (index, data) in ranges.iter().enumerate() {
                ...
                let chunk_size = img
                    ...
                    .expect("chunk_dims");
                // debug!("chunk_size: {chunk_size:?}, row_stride: {:?}", img.tile_opts.output_row_stride(i_32));
                ...
                let (first, rb) = rem_buf.split_at_mut(
                    usize::try_from(chunk_size.1).unwrap()
                        * img.tile_opts.output_row_stride(i_32).unwrap(),
                );
                rem_buf = rb;
                ...
                if let Err(e) = ChunkDecoder::expand_chunk(
                    data,
                    &mut first
                        .chunks_exact_mut(img.tile_opts.output_row_stride(i_32).unwrap())
                        .collect::<Vec<&mut [u8]>>(),
                    &img.tile_opts,
                    i_32,
                ) {
            ...
            let dyn_img = match t.as_str() {
                "rgba" => DynamicImage::from(
                    ImageBuffer::<Rgba<u8>, Vec<u8>>::from_raw(
                        img.tile_opts.image_width,
                        img.tile_opts.image_height,
                        img_buf,
                    )
                    .unwrap(),
                ),
