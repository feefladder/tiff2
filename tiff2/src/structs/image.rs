...
use crate::loader::tile::TileOpts;
...
impl Debug for Image {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("Image")
            .field("ifd", &self.ifd)
            .field("tile_opts", &self.tile_opts)
            .field(
                "chunk_offsets",
...
impl Image {
    ...
    /// get [`TileOpts`] of image
    pub fn tile_opts(&self) -> Arc<TileOpts> {
        self.tile_opts.clone()
    }
    ...
    pub fn from_ifd(mut ifd: Ifd, byte_order: ByteOrder) -> TiffResult<Image> {
        ...
        let tile_opts = Arc::new(TileOpts {
            byte_order,
            image_width,
            image_height,
